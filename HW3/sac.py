...
class SACAgent:
    ...
    def update_model(self) -> Tuple[torch.Tensor, ...]:
        ...
        with torch.no_grad():
            next_act, next_log_prob = self.actor(next_state)
            q1_next = self.qf_1(next_state, next_act)
            q2_next = self.qf_2(next_state, next_act)
            min_q_next = torch.min(q1_next, q2_next)
            q_target = reward + mask * self.gamma * (min_q_next - alpha.detach() * next_log_prob)
        q1_pred = self.qf_1(state, action)
        q2_pred = self.qf_2(state, action)
        qf_1_loss = F.mse_loss(q1_pred, q_target)
        qf_2_loss = F.mse_loss(q2_pred, q_target)
        ...
        vf_target = (torch.min(q1_new, q2_new) - alpha * log_prob).detach()
        vf_loss = F.mse_loss(v_pred, vf_target)
        ########## End of Your Code ##########
